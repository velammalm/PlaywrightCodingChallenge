...
} from '../framework-setup/optional-parameter-types';
...
export async function waitForNetworkIdleState(): Promise<void> {
  await getPage().waitForLoadState('networkidle');
}

export async function waitForLoadState(): Promise<void> {
...
export async function clickAndWaitForURL(input: string | Locator, targetURL: string, options?: ClickOptions): Promise<void> {
  const timeout = options?.timeout || STANDARD_TIMEOUT;
  await Promise.all([click(input, options), getPage().waitForURL(targetURL)]);
  await getPage().waitForLoadState(options?.loadState || LOADSTATE, {
    timeout: timeout,
  });
}
...
export async function scrollToBottomPage(): Promise<void> {
