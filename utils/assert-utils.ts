...
import { ExpectOptions, ExpectTextOptions, SoftOption } from '../framework-setup/optional-parameter-types';
...
export async function expectElementToHaveCSS(
  ...
  const { locator, assert } = getLocatorAndAssert(input, options);
  await assert(locator, options).toHaveCSS(attribute, value, options);
}
